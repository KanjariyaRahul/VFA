...
$farmer_name = $_GET['f-name'];
$farmer_mobileno = $_GET['f-mobileno'];
$farmer_email = $_GET['f-email'];
$farmer_username = $_GET['f-username'];
$farmer_password = hash("md5", $_GET['f-password']);
...
if ($validate_Rows == 0) { 
    ...
    $mail_hase = hash("md5", $farmer_email);
    $sql = "INSERT INTO `farmer` (`name`, `mobile`, `email`, `email-status`, `mail_hash`, `state`, `district`, `taluka`, `village`, `pincode`, `username`, `password`) VALUES ('$farmer_name', '$farmer_mobileno', '$farmer_email', 'not-verified', '$mail_hase', NULL, NULL, NULL, NULL, NULL, '$farmer_username', '$farmer_password');";
    ...
    if ($result) {
        ...
        if ($result = mysqli_query($con, $sql)) {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                $response = array("status" => true, "data" => $rows, "mail_status", $msend);
                echo json_encode($response);
            } else {
                $response = array("status" => false, "data" => "While selecting records from database your username and passwords are not mathcing Please contact Admin : vfa2023@outlook.com");
                echo json_encode($response);
            }
        }
    } else {
