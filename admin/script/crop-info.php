<?php 
    ...
    try{
        if(mysqli_query($conn, $sql)){
              header("Location: ../add-new-crop-data.php?msg=record inserted");
        }
        else{
            echo "Record Was not Inserted";
            header("Location: ../add-new-crop-data.php?msg=record was not inserted ".mysqli_error($conn));
        }
    }
    catch(Exception $e){
        echo "Exception: " . $e->getMessage();
        header("Location: ../add-new-crop-data.php?msg=".$e->getMessage());
    }
