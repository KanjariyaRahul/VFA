...
if (isset($_POST['upload'])) {
    ...
    try {
        ...
        // Execute query
        if ($update_msg != null || $filename != null) {
            $result = mysqli_query($con, $sql);
            if ($result) {
                // Now let's move the uploaded image into the folder: image
                if (move_uploaded_file($tempname, $folder)) {
                    $msg = '<div class="alert alert-success alert-dismissible fade show" role="alert"> <strong>Success! </strong> Update sent succesfully... <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button> </div>';
                }
            }
        } else {
